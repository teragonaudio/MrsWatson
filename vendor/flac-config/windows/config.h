...
// TODO: Fix assert on Windows
// Ugh, not sure why VC compiler keeps complaining with "'assert': identifier
// not found" messages, but until we get them sorted out, we must disable
// the assertion macro by tricking the #ifndef compile guard. Hacky. =/
#define FLAC__ASSERT_H 1
#define FLAC__ASSERT(x)
#define FLAC__ASSERT_DECLARATION(x)

// We build a static-only library, so disable this (as it causes
// a ton of errors/warnings about DLL linkage)
...
// TODO: Windows has this, but it fails compilation because of declspec alignment
